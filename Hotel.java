...
public class Hotel {
    ...
    public ArrayList<ArrayList<Room>> floors = new ArrayList<>();
    ...
    public boolean makeReservation(Guest guest, Room room, Scanner console) {
        ...
        if (Guest.yesOrNoQuestions("Yes or No: ", console)) {
            ...
            System.out.println("Please type year, month and day of the reservation in this format (MM/DD/YY)\nFrom: ");
            String from = console.nextLine();
            System.out.println("\nTo:");
            String to = console.nextLine();
            if(room.setResDate(from, to)){
                guestList.add(guest);
    ...
    public double totalCost(Guest guest, Room room) {
        return Math.round((room.getCost(guest.isGovernment() || guest.isMilitary() || guest.isMembership()) + guest.costOfGuests())*100D)/100D;
    }
